...
```
<!-- JB: I think this example would be more effective if you wrote out all of the lines in long format rather than stacking functions within each other. This is how noobs actually write code. That strategy would provide an immediate visual contrast. I'll make the change here. If you could come up with even more manipulation to do, that'd be great and would add to the example.--> 
...
The `spread` function takes the key-value pairs and spread them out into their own columns. <!-- JB: Maybe more explanation here, like your explanation for gather? --> Now we're back to one row per team, so it's easier to see the success rate of each unit for all of the teams.
